...
<body>
    <nav id="sidebar" class="sidebar">
        <ul>
            ...
            <li>
                ...
                <ul class="sub-menu">
                  <div>
                    <li><a href="../quanlynhanvien/quanlynhanvien.php">Quản lý nhân viên</a></li>
                    <li  class="active_child"><a href="./quanlykhachhang.php#">Quản lý khách hàng</a></li>
                  </div>
                </ul>
            </li>
            <li>
    ...
    <main class="main_admin">
        ...
        <table border="1" cellpadding="8" cellspacing="0">
            ...
            <?php
            // Lọc chỉ lấy user_type = 'customer'
            $result = $mysqli->query("SELECT * FROM users WHERE user_type = 'customer' ORDER BY user_id ASC");
            ...
            <tr>
                ...
                <td>
                    <?php if (!empty($row['img'])): ?>
                        <img src="../../../../public/uploads/avatar/<?= htmlspecialchars($row['img']) ?>" alt="Ảnh đại diện" width="60" height="60">
                    <?php else: ?>
                        Không có ảnh
                    <?php endif; ?>
                </td>
                ...
                <td class="action-link">
                    <?php if ($row['is_active'] == 1): ?>
                        <a class="ban" href="./camuser.php?id=<?= $row['user_id'] ?>" onclick="return confirm('Bạn có chắc muốn cấm tài khoản này?')">🚫 Cấm</a>
                    <?php else: ?>
                        <a class="unban" href="./mokhoauser.php?id=<?= $row['user_id'] ?>" onclick="return confirm('Mở lại tài khoản này?')">🔓 Mở cấm</a>
                    <?php endif; ?>
                    <a class="delete" href="./xoakhachhang.php?id=<?= $row['user_id'] ?>" onclick="return confirm('Bạn có chắc muốn xóa tài khoản này?')">❌ Xóa</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </table>

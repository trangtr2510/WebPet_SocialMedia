...
$id = $_GET['id'] ?? null;
if (!$id) {
    die("Thiếu ID danh mục cần sửa.");
}

// Lấy danh sách danh mục cha
$parentCategories = [];
$parentQuery = $conn->prepare("SELECT category_id, category_name FROM categories WHERE parent_category_id IS NULL AND category_id != ?");
$parentQuery->bind_param("i", $id); // Không cho chọn chính nó làm danh mục cha
$parentQuery->execute();
$parentQuery->bind_result($pid, $pname);
while ($parentQuery->fetch()) {
    $parentCategories[] = ['id' => $pid, 'name' => $pname];
}
$parentQuery->close();

// Lấy thông tin danh mục hiện tại
$stmt = $conn->prepare("SELECT category_name, description, parent_category_id, is_active FROM categories WHERE category_id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($name, $desc, $parent_id, $active);
$stmt->fetch();
$stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $new_parent = $_POST['parent_category_id'] ?: NULL;
    $new_active = isset($_POST['is_active']) ? intval($_POST['is_active']) : 1;

    $update = $conn->prepare("UPDATE categories SET category_name = ?, description = ?, parent_category_id = ?, is_active = ? WHERE category_id = ?");
    $update->bind_param("ssiii", $new_name, $new_desc, $new_parent, $new_active, $id);
    $update->execute();
    $update->close();
    ...
    <main class="main_admin">
        ...
        <form method="post" action="">
            <div class="form-group">
                ...
                <input type="text" name="category_name" id="category_name" required value="<?= htmlspecialchars($name) ?>" />
            </div>

            <div class="form-group">
                <label for="description">Mô tả:</label>
                <textarea name="description" id="description" rows="3"><?= htmlspecialchars($desc) ?></textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="parent_category_id" id="parent_category_id">
                    ...
                    <?php foreach ($parentCategories as $cat): ?>
                        <option value="<?= $cat['id'] ?>" <?= ($cat['id'] == $parent_id) ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                <button class="btn-cap-nhat">Cập nhật</button>
            </div>
        </form>
    </main>

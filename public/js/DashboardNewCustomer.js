class DashboardNewCustomer {
    constructor() {
        this.apiUrl = '../../app/controllers/NewCustomerController.php';
    ...
    async updateStats() {
        try {
            ...
            console.error('Error updating new customer stats:', error);
    ...
    updateUI(data) {
        ...
        const quantityElement = document.querySelector('.sales-customers h3');
        if(quantityElement) {
            quantityElement.textContent = data.current_total;
        }
        
        // Cập nhật phần trăm thay đổi
        const changeElement = document.querySelector('.sales-customers span');
        if(changeElement) {
        ...
        const titleElement = document.querySelector('.sales-customers p');
        if(titleElement && data.selected_period) {
            titleElement.textContent = `New Customers - ${data.selected_period}`;
        }
        
        // Trigger custom event để các component khác có thể lắng nghe
        document.dispatchEvent(new CustomEvent('newCustomersUpdated', {
            detail: data
        }));
    }
...
<style>
.sales-customers span.positive {
    color: #4CAF50;
}

.sales-customers span.negative {
    color: #f44336;
}

.sales-customers span.error {
    color: #ff9800;
    font-style: italic;
}
...
.sales-customers:hover {
    transform: translateY(-2px);
    box-shadow: 0 4px 12px rgba(32, 174, 243, 0.3);
}
...
// Thêm CSS vào head
document.head.insertAdjacentHTML('beforeend', newCustomerStyles);

// Khởi tạo Dashboard New Customer khi DOM đã sẵn sàng
document.addEventListener('DOMContentLoaded', function() {
    window.dashboardNewCustomer = new DashboardNewCustomer();
    
    // Expose một số method để sử dụng từ bên ngoài
    window.refreshNewCustomer = () => window.dashboardNewCustomer.refresh();
    window.getCurrentNewCustomerStats = () => window.dashboardNewCustomer.getCurrentStats();
});

// Auto-refresh mỗi 5 phút (tùy chọn)
setInterval(() => {
    if(window.dashboardNewCustomer && document.visibilityState === 'visible') {
        window.dashboardNewCustomer.refresh();
    }
}, 5 * 60 * 1000);